...
pkgdatadir = $(datadir)/joashcoincore
pkgincludedir = $(includedir)/joashcoincore
pkglibdir = $(libdir)/joashcoincore
pkglibexecdir = $(libexecdir)/joashcoincore
...
CONFIG_HEADER = $(top_builddir)/src/config/joashcoin-config.h
CONFIG_CLEAN_FILES = libjoashcoinconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libjoashcoinconsensus.pc.in \
	...
	$(top_srcdir)/src/config/joashcoin-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/JoashCoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/JoashCoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/JoashCoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/JoashCoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = joashcoin-cli
BITCOIN_DAEMON_NAME = joashcoind
BITCOIN_GUI_NAME = joashcoin-qt
BITCOIN_TX_NAME = joashcoin-tx
...
COPYRIGHT_HOLDERS_FINAL = The JoashCoin Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = JoashCoin Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/JoashCoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = joashcoincore
PACKAGE_BUGREPORT = https://github.com/joashcoinpay/joashcoin/issues
PACKAGE_NAME = JoashCoin Core
PACKAGE_STRING = JoashCoin Core 0.12.3
PACKAGE_TARNAME = joashcoincore
PACKAGE_URL = https://joashcoin.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/JoashCoin
abs_srcdir = /tmp/JoashCoin
abs_top_builddir = /tmp/JoashCoin
abs_top_srcdir = /tmp/JoashCoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/JoashCoin/build-aux/install-sh
...
pkgconfig_DATA = libjoashcoinconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/joashcoin-cli.bash-completion \
	       $(top_srcdir)/contrib/joashcoin-tx.bash-completion \
	       $(top_srcdir)/contrib/joashcoind.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_joashcoin_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_joashcoin_coverage.info test_joashcoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/JoashCoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/joashcoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/joashcoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/joashcoin-config.h
$(top_srcdir)/src/config/joashcoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/joashcoin-config.h src/config/stamp-h1
libjoashcoinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libjoashcoinconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/JoashCoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/JoashCoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/JoashCoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_joashcoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_joashcoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_joashcoin_filtered.info: test_joashcoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_joashcoin_filtered.info
...
#test_joashcoin_coverage.info: baseline_filtered_combined.info test_joashcoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_joashcoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_joashcoin_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_joashcoin_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_joashcoin.coverage/.dirstamp:  test_joashcoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_joashcoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_joashcoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
