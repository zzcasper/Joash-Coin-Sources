...
const char * const BITCOIN_CONF_FILENAME = "jnitacoin.conf";
const char * const BITCOIN_PID_FILENAME = "jnitacoind.pid";
...
#else
    const char* pszModule = "jnitacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JNitaCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JNitaCoin";
#else
    // Unix
    return pathRet / ".jnitacoin";
#endif
#endif
}
