...
const char * const BITCOIN_CONF_FILENAME = "joashcoin.conf";
const char * const BITCOIN_PID_FILENAME = "joashcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "joashcoin" is a composite category enabling all JoashCoin-related debug output
                if(ptrCategory->count(std::string("joashcoin"))) {
...
#else
    const char* pszModule = "joashcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JoashCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JoashCoin
    // Mac: ~/Library/Application Support/JoashCoin
    // Unix: ~/.joashcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JoashCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JoashCoin";
#else
    // Unix
    return pathRet / ".joashcoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty joashcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override joashcoin.conf
