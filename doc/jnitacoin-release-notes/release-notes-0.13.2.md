JNitaCoin Core version 0.13.2 is now available from:

  <https://download.jnitacoin.org/jnitacoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a jnitacoin
...
ECDSA signatures inside JNitaCoin transactions now use validation using
...
Previous versions of JNitaCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of JNitaCoin Core was to raise the default minimum
relay fee.

JNitaCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
JNitaCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running jnitacoind without having to do any manual configuration.
...
JNitaCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. JNitaCoin Core will relay transactions with
...
entirely in JNitaCoin Core 0.13, and it is at this time undecided whether the
...
JNitaCoin Core automatically creates a hidden service to listen on, without
manual configuration. JNitaCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if JNitaCoin Core is listening, and
...
JNitaCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals JNitaCoin
Core to use floating fees. By default, JNitaCoin Core will use floating
fees.
...
(default: `0.0002` JTK/kB).

At all times, JNitaCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) JTK.
Furthermore, JNitaCoin Core will never create transactions paying less than
...
`jnitacoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 25479 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 25479 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /jnitacoinrpc>
        ProxyPass http://127.0.0.1:5479/
        ProxyPassReverse http://127.0.0.1:5479/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass jnitacoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< jnitacoinrpc:<password>
    </Location>
...
Restarting jnitacoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in jnitacoin.conf
...
jnitacoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/jnitacoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The JNitaCoin Core code base has
...
- `jnitacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `jnitacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the JNitaCoin scripting system that in combination with
...
JNitaCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of JNitaCoin Core.
...
The two phases are distinct in the JNitaCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming JNitaCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for JNitaCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each JNitaCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of jnitacoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of jnitacoins being spent is exactly
...
- **Increased security for multisig:** JNitaCoin addresses (both P2PKH addresses
  ...
  believed to be provided by JNitaCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of JNitaCoin's current ledger (called the UTXO set),
  ...
  note that JNitaCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  JNitaCoin users to individually opt-in to almost any change in the JNitaCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and JNitaCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the JNitaCoin
client to JNitaCoin Core 0.13.2 to signal for segwit support. When a
...
Since JNitaCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `jnitacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `jnitacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
